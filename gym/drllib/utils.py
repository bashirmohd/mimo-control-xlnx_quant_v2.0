        ...
        while True:
            ...
            for idx, state in enumerate(states):
                if state is None:
                    ...
                    states_input.append(state.ravel())  # flatten out array in observation space
...
class ExperienceSourceFirstLast(ExperienceSource, object):  # added object to adapt to py2.7
    ...
    trajectory in replay buffer when we need only first and last states.
    For every trajectory piece it calculates discounted reward and emits only
    first and last states and action taken in the first state.  If we have partial
    trajectory at the end of episode, last_state will be None
    """
...
class TBMeanTracker:
    """
    TensorBoard value tracker:
    allows to batch fixed amount of historical values and write their mean into
    TB Designed and tested with pytorch-tensorboard in mind
    """
...
class RewardTracker:
    ...
    def reward(self, reward, frame, epsilon=None):
        ...
        logger.info("reward: {:8.1f},  mean_reward: {:8.1f}".format(reward, mean_reward))
