...
class DDPGCritic(nn.Module):
    def __init__(self, obs_size, act_size, features1=400, features2=300):
        ...
        self.obs_net = nn.Sequential(nn.Linear(obs_size, features1), nn.ReLU(),)

        self.out_net = nn.Sequential(nn.Linear(features1 + act_size, features2), nn.ReLU(), nn.Linear(features2, 1))
...
class AgentDDPG(BaseAgent):
    ...
    # ou_teta=0.15, ou_sigma=0.2 original ddpg
    """
    ...
    def __call__(self, states, agent_states):
        ...
        if self.ou_enabled and self.ou_epsilon > 0:
            ...
            for a_state, action in zip(agent_states, actions):
                ...
                actions = np.clip(actions, -1, 1)  # normalized action space
